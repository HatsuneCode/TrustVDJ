...
Read10x = function(airr_file      = NULL,
  ...
  if(is.null(airr) && is.null(contig)) {
    ...
    # no files
    if(is.null(consensus) & is.null(clonotype)) stop('!!! ', timer(), ' All input files does not exist, please check input file path !!!')
...
#' @param count_clonotype_freq logical. Count the frequency of clonotypes. Default \code{TRUE}
...
Read10xs = function(airr_files = NULL, contig_files = NULL, consensus_files = NULL, clonotype_files = NULL,
                    ...
                    unique_clonotype = TRUE, unique_clonotype_by_cdr3nt = TRUE, count_clonotype_freq = TRUE, 
  ...
  consenS = setNames(lapply(seq(nfile), function(i) {
    ...
    consen    = Read10x(airr_file = airr_files[i], contig_file = contig_files[i],
                        ...
                        verbose = verbose)
    clonoName = checkIn(clonoNames, colnames(consen))
    conseName = checkIn(conseNames, colnames(consen))
    if(length(clonoName)) {
  ...
  consenG = setNames(lapply(names(group), function(n) {
    ...
    clono_gp  = do.call(rbind, lapply(seq(consen_gp), function(i) {
      ...
      cdr3ntName = checkIn(cdr3ntNames, colnames(consen))
      cdr3aaName = checkIn(cdr3aaNames, colnames(consen))
      ...
      if(length(cdr3Name))
        do.call(rbind, lapply(unique(consen[[clonoName]]), function(clo) {
          nt = sort(unique(consen[[cdr3Name]][consen[[clonoName]] %in% clo]))
          nt = nt[!nt %in% 'None' & !is.na(nt)]
          if(length(nt)) 
            data.frame(sample = sample, id = clo, nt = gsub(';.*?:', ';', gsub('^.*?:', '', paste(nt, collapse = ';'))))
        }))
    }))
    ...
    if (unique_clonotype & nrow(dup_clono)) {
      ...
      for (nt in unique(dup_clono$nt)) {
        ...
        for (i in 2:nrow(dup_id)) {
          ...
          clonoName = checkIn(clonoNames, colnames(consen_gp[[sample]]))
          conseName = checkIn(conseNames, colnames(consen_gp[[sample]]))
          # dup -> replace
          d_clo  = dup_id$id[i]
