...
showCdr3Length = function(vdj, names = NULL, outpref = NULL, type = NULL, chain = NULL, plot = TRUE, compare = TRUE, save = TRUE) {
  ...
  outpref = as.character(outpref %|||% paste(names, collapse = '-') )
  ...
  if (save) {
    ...
    write.table(Len, paste0('cdr3Length/', outpref, '.cdr3Length.txt'), sep = '\t', row.names = F, quote = F)
    if (compare & nrow(Comp))
      write.table(Comp, paste0('cdr3Length/', outpref, '.cdr3Length.compare.txt'), sep = '\t', row.names = F, quote = F)
  }
  ...
  if (plot) {
    ...
    if (save) {
      ggsave(paste0('cdr3Length/', outpref, '.cdr3Length.num.pdf') , p1, width = 7, height = 6)
      ggsave(paste0('cdr3Length/', outpref, '.cdr3Length.prop.pdf'), p2, width = 7, height = 6)
    }
  }
